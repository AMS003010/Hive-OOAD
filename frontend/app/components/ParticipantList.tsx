...
export default function ParticipantList ({event}: comProps) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/search/participant`, {
                    method: 'POST',
                ...
                if (!response.ok) {
                    console.log("Unable get volunteers of the event");
                    return;
                }
                ...
                setParticipant(data)
                console.log("Fetched volunteers successfully");
    ...
    return(
        <div className="board-layout p-8 h-[31rem] mb-8 border border-gray-200 mx-4 rounded-3xl overflow-hidden shadow-xl overflow-y-auto">
            <div className="text-4xl font-extralight">Participants</div>
            <div className="flex flex-col">
                    {participant ? (
                        participant.length!=0 ? (
                            participant.map((participant_obj: Participant) => {
                                const color = gradColor()
                                return (
                                    <div key={participant_obj.id} className="flex justify-start items-center gap-4 bg-white cursor-pointer rounded-2xl p-4 mt-4 shadow-md transform transition-transform hover:-translate-y-2 hover:shadow-lg">
                                        <div
                                            style={{
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <div className="text-2xl font-light">{participant_obj.name}</div>
                                            <div className="flex justify-start items-center gap-2">
                                                <span className='p-1.5 text-xs font-medium text-blue-80 bg-blue-200 rounded-lg bg-opacity-50'>{participant_obj.email}</span>
                                            </div>
                                        </div>
                                    </div>
                                )
