...
export default function VolunteerList ({event}: comProps) {
    ...
    return(
        <div className="board-layout p-8 h-[31rem] mb-8 border border-gray-200 mx-4 rounded-3xl overflow-hidden shadow-xl overflow-y-auto">
            <div className="text-4xl font-extralight">Volunteers</div>
            <div className="flex flex-col">
                    {volunteers ? (
                        volunteers.length!=0 ? (
                            volunteers.map((volunteer_obj: Volunteer) => {
                                const color = gradColor()
                                return (
                                    <div key={volunteer_obj.id} className="flex justify-start items-center gap-4 bg-white cursor-pointer rounded-2xl p-4 mt-4 shadow-md transform transition-transform hover:-translate-y-2 hover:shadow-lg">
                                        <div
                                            style={{
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <div className="text-2xl font-light">{volunteer_obj.name}</div>
                                            <div className="flex justify-start items-center gap-2">
                                                <span className='p-1.5 text-xs font-medium text-blue-80 bg-blue-200 rounded-lg bg-opacity-50'>{volunteer_obj.email}</span>
                                            </div>
                                        </div>
                                    </div>
                                )
